...
<script>
	$(document).ready(function(){
		...
		function verificar(){
			var cfac='Factura',cced='cedula', cperiodo='Periodos',ffac='f_fact', table = $('#conceptos_det'), total_rece='conceptos_total', valido=true;
			valido = valido && validar(cfac);
			valido = valido && IsNumber(cfac);
			...
			valido = valido && count_row(table,'una Factura');
				...
				if($(this).find('.dataTables_empty').text()==''){
					...
					if(objeto=='Facturas'){
						...
						.done(function(trans){
							...
							$('#f_ori_c').val(trans.FECHA_FACTURA);
						...
						.done(function(trans){
							$('#Factura_c').val(trans.CODIGO_ORIGEN);
							$('#Devolucion_c').val(trans.CODIGO);
							$('#cedula_n').val(trans.CALUMNO);
							$('#cedula_c').val(trans.CEDULA);
							$('#nombres_c').val(trans.ALUMNO);
							$('#Periodos_c').val(trans.CPERIODO);
		...
		$(document).on('click', '.btn', function(e){
			...
			switch (button){
				...
				break;
				case 'bt_new':
					window.location.href = "?mod=COBROS&submod=DEL_COBROS";
				break;
				case 'bt_save':
					if(verificar()){
									...
									<div class="tab-pane fade active in" id="fact">
										<div class="col-xs-6 col-md-4">
											...
											<div id="Factura_g" class="form-group input-group">
												...
												<input id="Factura_c" name="Factura_c" type="text" class="form-control numeric" value="{CODIGO_ORIGEN}" autofocus>
										...
										<div class="col-xs-6 col-md-4">
											<div id="f_fact_g" class="form-group input-group">
												<label class="control-label" for="f_fact_c">FECHA DEVOLUCION</label>
												<input id="f_fact_c" name="f_fact_c" type="text" class="form-control" value="{FECHA_FACTURA}" readonly>
											</div>
											<div id="f_orig_g" class="form-group input-group">
												<label class="control-label" for="f_ori_c">FECHA FACTURA</label>
												<input id="f_ori_c" name="f_ori_c" type="text" class="form-control" value="{FECHA_ORIGEN}" readonly>
											</div>
										</div>
									</div>
											...
											<div class="panel-body">
												...
												<div class="row show-grid">
													<div class="col-xs-6 col-sm-4">
														...
														<input id="conceptos_count_c" type="text" readonly value="{FECHA_FACTURA}" class="form-control">
													</div>
													<div class="col-xs-6 col-sm-4">
