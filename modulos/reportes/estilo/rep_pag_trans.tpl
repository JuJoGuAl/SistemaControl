	...
	$(document).ready(function(){
		...
		$(document).on('click', '.btn', function(e){
			...
			switch (button){
				...
				break;
				case 'bt_new':
					window.location.href = "?mod=REPORTES&submod=REP_PAG_TRANS";
				break;
				case 'bt_print':
				var report = (getUrlParameter('submod'));
					imprimir('./modulos/reportes/'+report+'.php?accion=print&alum='+$('#Alumnos_c').val()+'&ban='+$('#Bancos_c').val()+'&fec1='+$('#Fecha_ini_c').val()+'&fec2='+$('#Fecha_fin_c').val());
				break;
				case 'bt_exit':
					...
					<div class="panel-body">
						<form role="form" name="inscripcion_form" id="inscripcion_form" method="post" enctype="multipart/form-data">
							<div class="row">
								...
								<div class="col-xs-3 col-md-3">
									<div id="Bancos_g" class="input-group">
										<label class="control-label" for="Bancos_c">BANCO</label>
										<input id="Bancos_c" name="Bancos_c" type="text" class="form-control hidden">
										<input id="Bancos_n" name="Bancos_n" type="text" class="form-control" readonly>
										<span class="input-group-btn"><button id="Bancos_btn" class="btn btn-default search_data" type="button"><i class="fa fa-search"></i></button></span>
									</div>
								</div>
								<div class="col-xs-3 col-md-3">
