...
if(empty($perm_val)){
	...
	if($action==''){
		...
		$tpl->assign("form_title",'CREAR UN CONVENIO');
		$tpl->assign("form_subtitle",'LOS CONVENIOS SE UTILIZAN PARA CLASIFICAR LAS INSCRIPCIONES DE LOS ALUMNOS (BECADOS / PLAN DE PAGO)');
	}elseif($action=='edt'){
		$tpl->assign("accion",'save_edt');
		$tpl->assign("form_title",'EDITAR UN CONVENIO');
		$tpl->assign("form_subtitle",'ACTUALICE LOS DATOS DEL CONVENIO');
		$tpl->assign("code",$_GET['id']);
		$data=$convenios->get_con($_GET['id']);
		if(!empty($data)){
