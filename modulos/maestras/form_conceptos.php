...
if(empty($perm_val)){
	...
	if($action==''){
		...
		$tpl->assign("form_title",'CREAR UN CONCEPTO');
		$tpl->assign("form_subtitle",'LOS CONCEPTOS SON UTILIZADOS PARA GENERAR UN COBRO.');
	}elseif($action=='edt'){
		$tpl->assign("accion",'save_edt');
		$tpl->assign("form_title",'EDITAR UN CONCEPTO');
		$tpl->assign("form_subtitle",'ACTUALICE LOS DATOS DEL CONCEPTO');
		$tpl->assign("code",$_GET['id']);
		$data=$conceptos->get_concep($_GET['id']);
		if(!empty($data)){
