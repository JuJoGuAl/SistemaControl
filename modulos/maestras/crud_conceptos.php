...
if(empty($perm_val)){
	...
	if($action==''){
		...
		$data=$conceptos->list_concep(false,false,'0');
		if(!empty($data)){
			foreach ($data as $llave => $datos) {
				...
				<button type="button" class="btn btn-default btn-circle modal-btn" data-action="edt" data-id="'.$id.'" data-toggle="tooltip" data-placement="top" title="" data-original-title="EDITAR CONCEPTO" href="form_conceptos" data-toggle="modal"><i class="fa fa-edit"></i></button>
				</div>
				';
