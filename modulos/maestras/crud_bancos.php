...
if(empty($perm_val)){
	...
	if($action==''){
		...
		$data=$bancos->list_bank(false,false);
		if(!empty($data)){
			foreach ($data as $llave => $datos) {
				...
				<button type="button" class="btn btn-default btn-circle modal-btn" data-action="edt" data-id="'.$id.'" data-toggle="tooltip" data-placement="top" title="" data-original-title="EDITAR BANCO" href="form_bancos" data-toggle="modal"><i class="fa fa-edit"></i></button>
				</div>
				';
